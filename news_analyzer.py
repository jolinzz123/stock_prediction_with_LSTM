...
def get_time_weight(timestamp: int | float) -> float:
    """Decay weight based on how many hours ago an article was published."""
    try:
...
def get_alphavantage_api_key() -> str | None:
    """Read the AV API key from the environment or a local .env file."""
    direct_value = (
...
def _alpha_vantage_query(**params) -> dict:
    ...
    message = (
        data.get("Information") or data.get(
...
def _summarize_alpha_vantage_feed(
    ...
    for article in feed[:max_items]:
        ...
        score, relevance = _extract_ticker_score(article, ticker)
        ...
        # Combine time-decay weight with AV relevance score
        time_weight = get_time_weight(
            _parse_published_at(article).timestamp()
            if _parse_published_at(article) is not None else 0
        )
        ...
        articles.append(
            {
                ...
                "url": article.get("url", ""),
                "published_at": format_publish_time(
    ...
    confidence = abs(avg_score) * min(len(scores) / 10, 1.0)
...
# Historical sentiment series (AV only)
...
def get_historical_sentiment_series(
    ...
    """
    Return a daily sentiment series aligned to *price_index* for use as a
    model feature.  Returns all-zeros when no AV key is available.
    """
    if not isinstance(price_index, pd.DatetimeIndex):
        aligned_index = pd.to_datetime(price_index)
    else:
    ...
    end = (aligned_index.max().normalize() + timedelta(days=1)).replace(
        hour=23, minute=59
    )

    try:
        feed = fetch_alpha_vantage_news_feed(
            ticker, limit=article_limit, start=start, end=end, sort="LATEST"
        )
...
def get_ticker_sentiment_context(
    ...
    if not get_alphavantage_api_key():
        return {
            ...
            "sentiment_series": pd.Series(
                0.0, index=DatetimeIndex(aligned_index.normalize()), dtype=float
            ),
        }
    ...
    try:
        ...
        return {
            "news_result": _summarize_alpha_vantage_feed(
                ticker, feed, max_items=max_items
            ),
            "sentiment_series": _build_daily_sentiment_series_from_feed(
                ticker, feed, aligned_index
            ),
        }
    except Exception:
        return {
...
def _get_yfinance_news_sentiment(ticker: str, max_items: int = 10) -> dict:
    ...
    for item in news_items[:max_items]:
        ...
        title = (
            item.get("title", "") or content.get("title", "")
        ).strip()
        if not title:
            continue
    ...
    avg_score = weighted_sum / total_weight if total_weight > 0 else 0.0
    confidence = abs(avg_score) * min(len(raw_scores) / 10, 1.0)
...
def get_news_sentiment(ticker: str, max_items: int = 10) -> dict:
    ...
    Preferred source : Alpha Vantage NEWS_SENTIMENT (ticker-level scores).
    Fallback source  : yfinance headlines scored with VADER + time-decay.
    """
    try:
        if get_alphavantage_api_key():
            feed = fetch_alpha_vantage_news_feed(
                ticker, limit=max(max_items, 20), sort="LATEST"
            )
...
def extract_market_drivers(news_result: dict, top_n: int = 5) -> list[dict]:
    """Return the *top_n* headlines with the strongest absolute sentiment."""
    return sorted(
