...
def get_stock_data(ticker):
    ...
    try:
        ...
        if history is not None and len(history) > 0:
            ...
            stock_data["historical_prices_30d"] = history["Close"].tail(
                30).tolist()
        else:
    ...
    try:
        if history is not None and len(history) > 0:
            ...
            if future_preds is not None:
                pred_list = future_preds.tolist() if isinstance(
                    future_preds, np.ndarray) else list(future_preds)
            ...
            if y_test is not None and test_preds is not None and len(y_test) > 0:
                stock_data["backtest_mse"] = float(
                    mean_squared_error(y_test[:, 0], test_preds))
            else:
    ...
    try:
        ...
        if sentiment is not None:
            stock_data["sentiment_score"] = float(
                sentiment.get("aggregate_score", 0.0))
        else:
...
def compute_scores(data_a, data_b):
    ...
    try:
        aok = data_a.get("predicted_price_7d") is not None and data_a.get(
            "current_price") is not None
        bok = data_b.get("predicted_price_7d") is not None and data_b.get(
            "current_price") is not None
        if aok and bok:
            ra = (data_a["predicted_price_7d"][-1] -
                  data_a["current_price"]) / data_a["current_price"] * 100
            rb = (data_b["predicted_price_7d"][-1] -
                  data_b["current_price"]) / data_b["current_price"] * 100
            w = "a" if ra > rb else ("b" if rb > ra else "tie")
            scores["predicted_return"] = {"a": round(ra, 2), "b": round(rb, 2), "winner": w, "label": "Predicted Return (7d)", "norm_a": _norm_pct(
                ra), "norm_b": _norm_pct(rb), "stars_a": _to_stars(_norm_pct(ra)), "stars_b": _to_stars(_norm_pct(rb))}
        else:
            scores["predicted_return"] = {
                "a": "N/A", "b": "N/A", "winner": "tie", "label": "Predicted Return (7d)"}
    except Exception as e:
        print(f"Error: {e}")
        scores["predicted_return"] = {
            "a": "N/A", "b": "N/A", "winner": "tie", "label": "Predicted Return (7d)"}
    ...
    try:
        ...
        if va is not None and vb is not None:
            ...
            scores["volatility"] = {"a": round(va, 3), "b": round(vb, 3), "winner": w, "label": "Risk (Volatility)", "norm_a": _norm_inverse(
                va), "norm_b": _norm_inverse(vb), "stars_a": _to_stars(_norm_inverse(va)), "stars_b": _to_stars(_norm_inverse(vb))}
        else:
            scores["volatility"] = {"a": "N/A", "b": "N/A",
                                    "winner": "tie", "label": "Risk (Volatility)"}
    except Exception as e:
        print(f"Error: {e}")
        scores["volatility"] = {"a": "N/A", "b": "N/A",
                                "winner": "tie", "label": "Risk (Volatility)"}
    ...
    try:
        ...
        if ma is not None and mb is not None:
            ...
            scores["model_confidence"] = {"a": round(ma, 4), "b": round(
                mb, 4), "winner": w, "label": "Model Confidence (MSE)"}
            ref = max(ma, mb) * 2
            if ref > 0:
                scores["model_confidence"]["norm_a"] = round(
                    max(0, (1 - ma / ref)) * 100, 1)
                scores["model_confidence"]["norm_b"] = round(
                    max(0, (1 - mb / ref)) * 100, 1)
            else:
                scores["model_confidence"]["norm_a"] = 50.0
                scores["model_confidence"]["norm_b"] = 50.0
            scores["model_confidence"]["stars_a"] = _to_stars(
                scores["model_confidence"]["norm_a"])
            scores["model_confidence"]["stars_b"] = _to_stars(
                scores["model_confidence"]["norm_b"])
        else:
            scores["model_confidence"] = {
                "a": "N/A", "b": "N/A", "winner": "tie", "label": "Model Confidence (MSE)"}
    except Exception as e:
        print(f"Error: {e}")
        scores["model_confidence"] = {
            "a": "N/A", "b": "N/A", "winner": "tie", "label": "Model Confidence (MSE)"}
    ...
    try:
        ...
        if sa is not None and sb is not None:
            ...
            scores["sentiment"] = {"a": round(sa, 3), "b": round(sb, 3), "winner": w, "label": "News Sentiment", "norm_a": _norm_pct(
                sa, lo=-1, hi=1), "norm_b": _norm_pct(sb, lo=-1, hi=1), "stars_a": _to_stars(_norm_pct(sa, lo=-1, hi=1)), "stars_b": _to_stars(_norm_pct(sb, lo=-1, hi=1))}
        else:
            scores["sentiment"] = {"a": "N/A", "b": "N/A",
                                   "winner": "tie", "label": "News Sentiment"}
    except Exception as e:
        print(f"Error: {e}")
        scores["sentiment"] = {"a": "N/A", "b": "N/A",
                               "winner": "tie", "label": "News Sentiment"}
    ...
    try:
        ...
        if ta is not None and tb is not None:
            ...
            scores["price_trend"] = {"a": round(ta, 4), "b": round(tb, 4), "winner": w, "label": "Price Trend (Momentum)", "norm_a": _norm_pct(
                ta, lo=-2, hi=2), "norm_b": _norm_pct(tb, lo=-2, hi=2), "stars_a": _to_stars(_norm_pct(ta, lo=-2, hi=2)), "stars_b": _to_stars(_norm_pct(tb, lo=-2, hi=2))}
        else:
            scores["price_trend"] = {
                "a": "N/A", "b": "N/A", "winner": "tie", "label": "Price Trend (Momentum)"}
    except Exception as e:
        print(f"Error: {e}")
        scores["price_trend"] = {"a": "N/A", "b": "N/A",
                                 "winner": "tie", "label": "Price Trend (Momentum)"}
...
def determine_winner(ticker_a, ticker_b, scores):
    ...
    for dk, dd in scores.items():
        ...
        if ww == "a":
            sa += w
            breakdown.append(
                f"  +{w} {dd['label']}: {ticker_a} wins ({dd['a']} vs {dd['b']})")
        elif ww == "b":
            sb += w
            breakdown.append(
                f"  +{w} {dd['label']}: {ticker_b} wins ({dd['a']} vs {dd['b']})")
        else:
            sa += w / 2
            sb += w / 2
            breakdown.append(
                f"  +{w/2} {dd['label']}: Tie ({dd['a']} vs {dd['b']})")
    ...
    if sa > sb:
        ...
        if vol.get("winner") == "a" and isinstance(vol.get("a"), (int, float)):
            advices.append(
                f"{ticker_a} has lower volatility, suitable for conservative investors.")
        elif vol.get("winner") == "b" and isinstance(vol.get("b"), (int, float)):
            advices.append(
                f"{ticker_b} has lower volatility, suitable for conservative investors.")

        if ret.get("winner") == "a" and isinstance(ret.get("a"), (int, float)):
            advices.append(
                f"{ticker_a} has stronger growth forecast, better for growth-oriented investors.")
        elif ret.get("winner") == "b" and isinstance(ret.get("b"), (int, float)):
            advices.append(
                f"{ticker_b} has stronger growth forecast, better for growth-oriented investors.")
        ...
        reason = "Both stocks score equally. " + \
            (" ".join(advices) if advices else "Consider your own risk preference.")
        return {"winner": winner, "score_a": sa, "score_b": sb, "total": total, "reason": reason, "breakdown": breakdown}
...
def print_comparison_report(report):
    ...
    if wr["winner"] is not None:
        ...
        print(
            f"  Score:             {wr['score_a']:.1f} / {wr['total']} (Stock A)")
        print(
            f"                     {wr['score_b']:.1f} / {wr['total']} (Stock B)")
    else:
